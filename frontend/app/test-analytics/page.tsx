...
export default function TestAnalyticsPage() {
  ...
  useEffect(() => {
    ...
    if (getCurrentUser()?.github_username) {
      fetchGitHubStats();
    }
  }, []);
  ...
  const fetchGitHubStats = async () => {
    try {
      ...
      if (!currentUser?.github_username) return;

      // Fetch GitHub user data
      const response = await fetch(`https://api.github.com/users/${currentUser.github_username}`);
      if (response.ok) {
        const data = await response.json();
        
        // Calculate account age in years
        const accountAge = Math.floor((Date.now() - new Date(data.created_at).getTime()) / (1000 * 60 * 60 * 24 * 365));
