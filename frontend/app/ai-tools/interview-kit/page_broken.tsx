...
export default function InterviewKitPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      // Always use the comprehensive fallback template for reliable results
      const skillsList = selectedRole.skills.map(skill => `- **${skill}**: Describe your experience and a specific project where you used this`).join('\n');
      const topSkills = selectedRole.skills.slice(0, 3).join(', ');
      const dept = selectedRole.department.toLowerCase();
      const roleType = selectedRole.title.includes('Frontend') ? 'user interface' : 'system architecture';
      const designType = selectedRole.title.includes('Frontend') ? 'component architecture' : 'simple system';
      
      const interviewKitContent = `# Interview Kit: ${selectedRole.title}
...
- How would you approach building a scalable ${roleType}?
...
- **System Design**: Design a ${designType}
...
- [ ] Offer discussion`;
      ...
      setInterviewKit(interviewKitContent);
    } catch (error) {
